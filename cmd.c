...
int cmd(int new_fd) {
  ...
  printf("cmd received\n");
  printf("%s\n", t.buf);
  for (i = 0; t.buf[i] != ' ' && t.buf[i] != 0; i++) command[i] = t.buf[i];
  int pos = i + 1;
  for (j = 0; t.buf[pos + j] != 0; j++) parameters[j] = t.buf[pos + j];
  int cmd_idx = -1;
  ...
  switch (cmd_idx) {
    ...
    case 3 : transferFile2Client(new_fd, parameters); break;
...
void sig(int signum) { printf("sigpipe is coming\n"); }

int transferFile2Client(int new_fd, char *parameters) {
  printf("%s--------\n", parameters);
  train t;
  ...
  while (bzero(&t, sizeof(t)), t.flag = 2, (t.len = read(fd, t.buf, sizeof(t.buf))) > 0) { // 读1024个字节到train的buffer里面
    int ret = sendn(new_fd, (char *)&t, 8 + t.len); // 将tain里面的内容发到socket的对端， 8是len和flag的长度
    if (ret == -1) {
      goto end;
    }
  }
  t.len = 0;  //可以不加这句话，因为最后一次read返回0
  sendn(new_fd, (char *)&t, 4);  //高危处, 通过给client传一个len为0的小火车，告知本次传输结束。
end:
  close(fd);  //如果client掉线，server子进程关闭传文件的描述符，return
            //0，此时子程序在一直等待接收client的命令，
...
int receive_from_server(int socket_fd) {
  ...
  while (1) {
    recvn(socket_fd, (char *)&t.len, sizeof(t.len)); // 接收本次小火车，发送数据的len
    if (t.len > 0) { // 
